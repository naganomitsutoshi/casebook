...
## 22. Phase 2 差分（E2E 暗号化同期・v10.0 / SPEC-phase2）

- **任意機能**：`casebook:sync`（localStorage）が無ければ従来どおり完全ローカル・外部通信ゼロ。Firebase SDK は同期有効時のみ動的 import（gstatic 10.14.1）。§2 の「外部送信なし」はこの例外を含めて読む（送信されるのは AES-GCM 暗号文のみ・写真と端末別設定は対象外）
- **鍵**：PBKDF2(パスフレーズ, salt, 310k, SHA-256)→AES-GCM 256。salt と鍵確認暗号文は `users/{uid}/meta/crypto`。導出済み JWK は `casebook:sync` に保持（バックアップ JSON には**含めない**）
- **データ**：`users/{uid}/cases/{caseId}` = `{v, iv, blob, deleted, updatedAt}`。blob=暗号化された `{case, mt}`。完全削除は `deleted:true` の墓標（復活防止）
- **マージ**：症例トップレベルフィールド単位 LWW（`mt` はクライアント時刻・保存時に `syncAfterSave` が刻印＝オフライン中も正確）。純ロジックは logic ブロック `sync*` 関数群（verify-phase2 が2端末収束まで検証）
- **タイミング**：boot / visibilitychange / 保存成功5秒後 / 手動「今すぐ」。同期中の再入は SYNC_PENDING で直列化
- **注意**：リモート変更取込時は全画面 `render()` が走る（入力中なら稀にフォーカスが飛ぶ・許容済み）。`ensureCaseShape`/`rolloverCase` のフィールド保全ルール（§20）は同期経由の症例にも効く（取込後 `normalizeData` を通す）
- **キルスイッチ**：Sync 設定→無効化（ローカル・サーバのデータは残る）。サーバ側を消すなら Firebase コンソールで Firestore の `users/` を削除
